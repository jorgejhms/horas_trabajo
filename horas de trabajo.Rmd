...
pmdr$graph_diario(df, date(inicio), pomodoros, 90)
```
...
```{r}
pmdr$plot_trabajos(month, trabajo, pomodoros, T)
```

### En la última semana

```{r}
pmdr$plot_trabajos(week, trabajo, pomodoros, T)
```
...
df  |>
  ...
  pmdr$plot_trabajos(`wday(\`date(inicio)\`)` |> as.factor(), pomodoros) +
  coord_flip()
```
...
```{r}
pmdr$plot_barras(week, inicio, pomodoros, T)
week  |>
  group_by(date(inicio)) |>
  summarise(pomodoros = sum(pomodoros)) |>
  pmdr$plot_trabajos(`date(inicio)` |> as.factor(), pomodoros) +
  coord_flip()
```
